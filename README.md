...
| Дашборд владельца      | Вход через Telegram, живые диалоги (обновление 2,5 с), ответ клиенту, закрытие заявки |
...
| `401` / «Требуется вход» на дашборде     | Нажмите «Войти через Telegram» и подтвердите код ботом (`/login КОД`). Вход — только для владельца и админов из `ADMIN_TELEGRAM_IDS` |
...
- Дашборд закрыт входом через Telegram: владелец + назначенные им админы (`ADMIN_TELEGRAM_IDS`), назначения — в разделе «Администраторы».
